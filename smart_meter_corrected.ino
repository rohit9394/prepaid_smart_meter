...
#define VOLT_PIN  A0
#define CURR_PIN  A1
#define SS_PIN    8      // RFID SS (since LoRa likely uses D10)
#define RST_PIN   9
...
double calculateFrequency()
{
  ...
  while(crossings<6)   // reduced for speed
  {
...
void processPower()
{
  ...
  {
    loadState = true;
    double consumed = Power * dt / 3600.0;   // Wh → units
...
void updateLCD()
{
  ...
  lcd.setCursor(0,1);
  lcd.print("Bal:");
  lcd.print(energyBalance,2);

  if(loadState) lcd.print(" ON ");
  else lcd.print(" OFF");
}
...
void loop()
{
  ...
  // if(abs(energyBalance - lastWritten) > 0.1)
  // {
  //   writeCardEnergy();
  //   lastWritten = energyBalance;
  // }
